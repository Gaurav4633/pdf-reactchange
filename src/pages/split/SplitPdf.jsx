...
const SplitPdf = () => {
  ...
  /* ================= SPLIT LOGIC (SAME AS COMPRESS STYLE) ================= */
  ...
  const handleSplit = async () => {
    ...
    if (!pages.trim()) {
      alert("Please enter pages (example: 1-3 or 2,5)");
      return;
    }
    ...
    try {
      // ✅ SAME PATTERN AS compressPdf
      const pdfBlob = await api.splitPdf(selectedFile, pages);

      const url = window.URL.createObjectURL(pdfBlob);

      setResult({
        url,
        filename: selectedFile.name.replace(".pdf", "-split.pdf"),
      });
